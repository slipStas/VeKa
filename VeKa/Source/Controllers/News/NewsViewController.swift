...
extension NewsViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        switch self.newsArray[indexPath.row].likes.likeStatus {
        case .like:
        ...
        cell.newsLikeView.onTap = {
            if cell.newsLikeView.likesStatus == .noLike {
                self.newsArray[indexPath.row].likes.likesCounts += 1
                self.newsArray[indexPath.row].likes.likeStatus = .like
                cell.newsLikeView.likesStatus = .like
                self.newsTableView.reloadData()
            } else {
                self.newsArray[indexPath.row].likes.likesCounts -= 1
                self.newsArray[indexPath.row].likes.likeStatus = .noLike
                cell.newsLikeView.likesStatus = .noLike
                self.newsTableView.reloadData()
            }
        }
